...
const Earthquakes: React.FC<{ firstUrl: string, secondUrl: string }> = ({ firstUrl, secondUrl }) => {
  ...
  const generateImageUrl = (): string => {
    ...
    return `${firstUrl}${year}${month}${day}_${hours}${minutes}.png`;
  };
  ...
  const generateSecondImageUrl = (): string => {
    ...
    return `${secondUrl}${year}${month}${day}_${hours}${minutes}.png`;
  };
  ...
  return (
    <>
      <div style={{ textAlign: 'center', padding: '1rem' }}>
        <h2>Live Earthquakes in the Region</h2>
        {/* Conditional Rendering to avoid empty src */}
        <div>
          <div style={{ display: 'flex', justifyContent: 'center'}}>
            <div style={{ width: '400px'}}>
              {imageUrl ? (
                <ImagePopup placeholderSrc={imageUrl} imageSrc={imageUrl} altText='EQ map' titleText='earthquakes' />
              ) : (
            ...
            <div style={{ width: '400px'}}>
              {secondimageUrl ? (
                <ImagePopup placeholderSrc={secondimageUrl} imageSrc={secondimageUrl} altText='EQ graph' titleText='earthquakes' />
              ) : (
                <p>Waiting for updated image...</p>
              )}
            </div>
